...
% im=imread(full);
im = imread('img/IMG-20210724-WA0009.jpg');
figure(1)
...
im = edge(imgray, 'sobel');figure(5),%imshow(im)
im = imdilate(im, strel('diamond', 2));
im = imfill(im, 'holes');
im = imerode(im, strel('diamond', 10));imshow(im)
...
[h, w] = size(im);
figure(1),subplot(222), imshow(im);%title('Detected Number Plate');
...
fin=[district '-' type '-' noPlate]
 figure(1),subplot(222),title(fin)
